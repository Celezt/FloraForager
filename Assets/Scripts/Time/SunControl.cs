...
public class SunControl : MonoBehaviour
{
    ...
    private void Update()
    {
        _Pitch = 360.0f * ((_DateTime.HourClock + (_DateTime.MinuteClock / 60.0f)) / 24.0f) - _MorningOffset;

        float angleDiff = Mathf.Abs(_NewPitch) - Mathf.Abs(_Pitch);
        _NewPitch = (angleDiff < 45.0f) ? Mathf.SmoothStep(_NewPitch, _Pitch, Time.deltaTime * _Damping) : _Pitch;
